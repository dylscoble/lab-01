...
at https://github.com/dylscoble/lab-01
---
...
There are 739 total observations in the 'trails' dataset. In each observation, there are 12 variables.
...
current_trails <- trails %>%
  filter(status == "Existing", !is.na(trails))
...
ggplot(data = current_trails) +
  ...
  labs(x = "Length", 
       y = "# of Observations", 
       title = "San Francisco Current Trails")
...
current_trails %>%
  summarise(min = min(length), 
            q1 = mean(length)-(IQR(length)/2), 
            median = median(length), 
            q3 = mean(length)+(IQR(length)/2), 
            max = max(length), 
            iqr = IQR(length), 
            mean = mean(length), 
            std_dev = sd(length)
...
```{r eval = FALSE}
hikable_trails <- current_trails %>%
  filter(length <= 5, !is.na(current_trails))
glimpse(hikable_trails)
...
8. Consider the distribution of `class`. 
    ...
    'class' takes the value of an integer between 1 and 3. There are two missing observations of  class, which may indicate that these trails have not been assigned a class yet.
...
```{r eval=FALSE}
refined_class <- hikable_trails %>% filter(class !=3) %>% 
  mutate(class = if_else(is.na(class),1,class)) 
glimpse(refined_class)
```
...
```{r eval=FALSE}
ggplot(data = hikable_trails) +
  geom_point(mapping = aes(x = class, y = length)) + 
  labs(x = "Class", 
       y = "Length", 
       title = "San Francisco Current Trails: Length vs Class")
```
...
All trails longer than 2.5 miles are labelled as class 1 trails. All class two and three trails are smaller than 2.5 miles. There are only two bike routes, and both of them are less than one mile long.
...
Null Hypothesis: The average length of the trails with bike lanes (Class 2) is the same os the average length of trails that share bikes with pedestrians (Class 1).

Alternative Hypothesis: The average length of the trails with bike lanes (Class 2) is different than the  average length of trails that share bikes with pedestrians (Class 1).
...
t.test(length ~ class, data = refined_class, 
...
15. Use the output from the previous exercise to answer the following: 
    ...
    The test statistic represents the number of standard deviations away from the mean the  difference in means is, given that the null hypothesis is true.
    
    - Write the definition of the p-value in the context of this analysis.
    
    The p-value represents the probability of the test statistic.
    
    - State your conclusion in the context of this analysis. Use a significance level of $\alpha = 0.01$.
    
    We can conclude that there is not enough statistical evidence to reject the null hypothesis. In other words, There is not enough evidence to suggest that length and class have a linear relationship.
...
In a repeated sampling of size n from the total population, 99% of the difference in mean trail length was between -0.2434412 and 0.1405578. In other words, we are 99%  confident that the true population difference in mean trail length is between -0.2434412 and 0.1405578.
